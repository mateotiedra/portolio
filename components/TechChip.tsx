...
import { SiMui, SiReact, SiGooglecloud, SiNodedotjs, SiLua, SiTailwindcss, SiGoogle, SiPython, SiPytorch, SiNumpy, SiFlutter } from "react-icons/si";
...
function TechChip({ name, icon, url }: TechChipProps) {
    return <a className='cursor-pointer' target='_blank' rel="noreferrer" href={url}>
        <div className='flex flex-row gap-2 rounded-xl items-center justify-center px-2 py-1 border-[1px]'>
            <span className='uppercase' style={{ fontSize: '0.6rem' }}>{name}</span>
            {icon}
        </div>
    </a>
}
...
export const ConceptionChip = () => <TechChip name='Conception' icon={<TbTools size={15} color='#c211b3' />} />
